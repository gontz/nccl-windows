...
/* On Windows, winsock2.h must be included before windows.h (which cuda_runtime.h
 * pulls in).  Defining WIN32_LEAN_AND_MEAN prevents windows.h from including the
 * old winsock.h, avoiding sockaddr/fd_set redefinition errors in translation
 * units that also include socket networking headers. */
...
/* Cross-platform deprecated attribute */
#if defined(_MSC_VER)
#define NCCL_DEPRECATED(msg) __declspec(deprecated(msg))
#else
#define NCCL_DEPRECATED(msg) __attribute__((deprecated(msg)))
#endif
...
NCCL_DEPRECATED("ncclResetDebugInit is not supported as part of the NCCL API and will be removed in the future")
void  ncclResetDebugInit();
NCCL_DEPRECATED("pncclResetDebugInit is not supported as part of the NCCL API and will be removed in the future")
