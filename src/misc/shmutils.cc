...
ncclResult_t ncclShmOpen(char* shmPath, size_t shmPathSize, size_t shmSize, void** shmPtr, void** devShmPtr, int refcount, ncclShmHandle_t* handle) {
  ...
  *handle = *shmPtr = NULL;
...
#ifdef NCCL_OS_WINDOWS
  {
    ...
    if (create) {
      // NOTE: The cudaHostAlloc shortcut that was here has been removed.
      // It allocated anonymous (unnamed) pinned memory when shmPath was empty
      // and devShmPtr was requested, which works for same-process (legacy)
      // connections but breaks cross-process pipeline-parallel because the
      // other process has no named section to open.  Same-process connections
      // still work efficiently: shm.cc's shmSendConnect/shmRecvConnect bypass
      // ncclShmImportShareableBuffer entirely when legacy==true, reusing the
      // peer's hptr directly via cudaHostGetDevicePointer.
      if (shmPath[0] == '\0') {
        // Use a named page-file-backed section so that other processes can
        // open it by token via OpenFileMappingA.  shm.cc extracts the 6-char
